...
describe("testphonegeopromise", () => {
    test("PhoneGeoAsyncNANP", () => {
        expect.assertions(6);
        return PhoneNumber.create("+1 650 654 3210", {
            sync: false
        }).then(parsed => {
            return PhoneGeoLocator.create({
                sync: false
            }).then(locator => {
                var expected = {
                    country: {
...
//  for bug NOV-118981
    test("PhoneGeoAsyncNANPInvalidNumber", () => {
        expect.assertions(6);
        return PhoneNumber.create("1 234", {
            sync: false
        }).then(parsed => {
            return PhoneGeoLocator.create({
                locale: 'en-US',
    ...
    test("PhoneGeoAsyncDefaultDE", () => {
        expect.assertions(6);
        return PhoneNumber.create("06224 123456", {
            locale: "de-DE",
            sync: false
        }).then(parsed => {
            return PhoneGeoLocator.create({
    ...
    test("PhoneGeoAsyncDEMobileNumber", () => {
        expect.assertions(6);
        return PhoneNumber.create("017 12345678", {
            locale: "de-DE",
            sync: false
        }).then(parsed => {
            return PhoneGeoLocator.create({
    ...
    test("PhoneGeoAsyncDefaultHK", () => {
        expect.assertions(6);
        return PhoneNumber.create("0663 12345678", {
            locale: 'en-CN',
            sync: false
        }).then(parsed => {
            return PhoneGeoLocator.create({
