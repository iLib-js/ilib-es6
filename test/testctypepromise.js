...
export const testctypepromise = {
    ...
    testIsLowerTrue: function(test) {
        test.expect(11);
        isLower._init(false).then(function() {
            test.ok(isLower('a'));
            test.ok(isLower('m'));
            test.ok(isLower('щ'));
            test.ok(isLower('λ'));
    ...
    testIsUpperTrue: function(test) {
        test.expect(11);
        isUpper._init(false).then(function() {
            test.ok(isUpper('A'));
            test.ok(isUpper('M'));
            test.ok(isUpper('Щ'));
            test.ok(isUpper('Λ'));
    ...
    testIsPrintTrue: function(test) {
        test.expect(7);
        isPrint._init(false).then(function() {
            test.ok(isPrint(' '));
            test.ok(isPrint('A'));
            test.ok(isPrint('M'));
            test.ok(isPrint('Щ'));
    ...
    testIsAsciiTrue: function(test) {
        test.expect(13);
        isAscii._init(false).then(function() {
            test.ok(isAscii('a'));
            test.ok(isAscii('m'));
            test.ok(isAscii('z'));
            test.ok(isAscii('A'));
    ...
    testIsBlankTrue: function(test) {
        test.expect(13);
        isBlank._init(false).then(function() {
            test.ok(isBlank(' '));
            test.ok(!isBlank('a'));
            test.ok(!isBlank('m'));
            test.ok(!isBlank('z'));
    ...
    testIsSpaceTrue: function(test) {
        test.expect(11);
        isSpace._init(false).then(function() {
            test.ok(isSpace(' '));
            test.ok(isSpace('\t'));
            test.ok(isSpace('\n'));
            test.ok(isSpace('\u2000'));
    ...
    testIsPunctTrue: function(test) {
        test.expect(17);
        isPunct._init(false).then(function() {
            test.ok(isPunct('?'));
            test.ok(isPunct('.'));
    ...
    testIsIdeoTrue: function(test) {
        test.expect(10);
        isIdeo._init(false).then(function() {
            test.ok(isIdeo('碗'));
            test.ok(isIdeo('人'));
            test.ok(!isIdeo(' '));
            test.ok(!isIdeo('$'));
    ...
    testIsCntrlTrue: function(test) {
        test.expect(10);
        isCntrl._init(false).then(function() {
            test.ok(isCntrl('\u0001'));
            test.ok(isCntrl('\u0085'));
            test.ok(!isCntrl(' '));
            test.ok(!isCntrl('$'));
    ...
    testIsDigitTrue: function(test) {
        test.expect(20);
        isDigit._init(false).then(function() {
            test.ok(isDigit('0'));
            test.ok(isDigit('1'));
            test.ok(isDigit('2'));
            test.ok(isDigit('3'));
    ...
    testIsXdigitTrue: function(test) {
        test.expect(32);
        isXdigit._init(false).then(function() {
            test.ok(isXdigit('0'));
            test.ok(isXdigit('1'));
            test.ok(isXdigit('2'));
            test.ok(isXdigit('3'));
    ...
    testWithinRangeTrue: function(test) {
        test.expect(5);
        CType._init(false).then(function() {
    ...
    testIsScriptTrue: function(test) {
        test.expect(16);
        isScript._init(false).then(function() {
    ...
    testIsAlnum: function(test) {
        test.expect(11);
        isAlnum._init(false).then(function() {
            test.ok(isAlnum('a'));
            test.ok(isAlnum('m'));
            test.ok(isAlnum('z'));
            test.ok(isAlnum('A'));
    ...
    testIsGraphTrue: function(test) {
        test.expect(5);
        isGraph._init(false).then(function() {
            test.ok(isGraph('A'));
            test.ok(isGraph('Q'));
            test.ok(isGraph('碗'));
            test.ok(!isGraph(' '));
